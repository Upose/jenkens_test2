<template>
  <div class="content">
    <div class="left_cont">
      <div class="box">
        <!-- <div class="header">
					<span>{{ $t('common.warehouse_header') }}</span>
				</div> -->
...
import { warehouseApi } from '@/http/api/othcustom/stockset/config/warehouse'
  ...
  setup() {
    ...
    const state: IState = reactive({
      ...
      customArgs: {
        ...
        table: CUSTOM_TABLES.INVENTORY_WAREHOUSE
    ...
    const requests = {
      ...
      getUpd() {
        const data = dataStructure({ power_url: 'V1/InventoryWarehouse/upd' }, state.updForm)
        warehouseApi
          .get_upd(data)
          .then(res => {
      ...
      getAdd() {
        const data = dataStructure({ power_url: 'V1/InventoryWarehouse/add' }, state.addForm)
        warehouseApi
          .get_add(data)
          .then(res => {
            let { status, custom_data, info, field_name } = res as IRequest

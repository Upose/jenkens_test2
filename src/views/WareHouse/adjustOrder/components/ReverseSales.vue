 ...
 * @LastEditors: TJ
 * @LastEditTime: 2021-12-02 09:11:00
-->
<!--  -->
<template>
  <el-drawer title="销售单红冲" :size="800" v-model="showSales">
            ...
            <el-col :span="24">
              <el-form-item :label="$t('common.id')" prop="id">
                <el-select clearable filterable v-model="addForm.id" @change="idChange">
                  <el-option
                    v-for="(item, index) of commonLists.allSalesList"
                    :key="index"
                    :value="item.id"
                    :label="item.id"
                  ></el-option>
    ...
    const requests = {
      getSales() {
        const data = dataStructure(
          { power_url: 'V1/InventoryInformation/order_red_punch' },
          state.addForm
        )
        stocktakingApi
          .get_sales(data)
      ...
      getSalesNextSearch() {
        ...
        stocktakingApi
          .get_sales_next_search(data)
          .then(res => {
            let { status, custom_data, info } = res as IRequest
            if (status === 200) {
              custom_data.data.forEach((item: any) => {
                item.show_info = `${item.inventory_id}-
                ${item.sale_order_number}-${item.handle_number}-${item.sale_money}`
              })
              state.salesList = custom_data.data
            }
          })
          .catch(err => err)
      }

...
<template>
	<div class="tableInputStyle">
		<el-table :data="all_array" id="addTable" highlight-current-row border>
	...
	setup(props, ctx) {
		...
		const methods = {
			deleteField(index: number, rows: any) {
				rows.splice(index, 1)
				state.all_array = rows
				ctx.emit('setAddArray', rows)
			},
			initUpdSort() {
				const tbody: any = document.querySelector(
					'#addTable .el-table__body-wrapper tbody'
				)
				...
				Sortable.create(tbody, {
					onEnd: (evt: any) => {
						...
						// 清空数据，再通过nextTick赋值
						state.all_array = []
						ctx.emit('setAddArray', all_array)
						nextTick(() => {

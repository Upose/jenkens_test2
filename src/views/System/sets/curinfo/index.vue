...
import { curinfoApi } from '@/http/api/system/sets/curinfo'
...
export default defineComponent({
  setup() {
    ...
    const requests = {
      ...
      getAdd() {
        const data = dataStructure({ power_url: 'V1/Unit/add' }, state.addForm)
        curinfoApi
          .get_add(data)
          .then((res: any) => {
        ...
        return new Promise((resolve, reject) => {
          ...
          curinfoApi
            ...
            .then((res: any) => {
              ...
              if (status === 200) {
                const { id, name_cn, name_en, name_fr, is_enabled } = custom_data
                state.updForm = {
                  id,
                  name_cn,
                  name_en,
      ...
      // 货币单位修改（提交）接口：V1/UNit/upd
      getUpd() {
        const data = dataStructure({ power_url: 'V1/UNit/upd' }, state.updForm)
        curinfoApi
          .get_upd(data)
          .then((res: any) => {
            let { status, custom_data, info, field_name } = res as IRequest

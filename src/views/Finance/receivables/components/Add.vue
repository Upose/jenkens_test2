 ...
 * @LastEditors: HYH
 * @LastEditTime: 2022-05-27 10:41:56
-->
<!--  -->
<template>
    ...
    <div class="box-card formStyle dividerStyle">
      <div class="box-form">
        ...
        >
          <el-row>
            <el-col :span="12">
              <el-form-item :label="$t('common.collection_order_number')" prop="config_number">
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const requests = {
      // // 财务中心进货单财务付款ID生成接口：V1/ReceivablesManagement/config_number
      getConfigNumber() {
        const data = dataStructure({ power_url: 'V1/ReceivablesManagement/config_number' }, {})
        receivablesApi
          .get_config_number(data)
          .then(res => {
            let { status, custom_data, info } = res as IRequest
            if (status === 200) {
              state.addForm.config_number = custom_data?.config_number
            }
          })
          .catch(err => err)
      },
      ...
      getAdd() {
        ...
        const data = dataStructure(
          { power_url: 'V1/ReceivablesManagement/add' },
          {
            ...state.addForm,
            file: fileData,
    ...
    const methods = {
      ...
      updateTableData(index: any, deleting_file: any) {
        // 删除对应上传到后端的数据
        state.collectData.splice(index, 1)
        // 删除后台图片
        deleting_file.forEach((item: any) => {

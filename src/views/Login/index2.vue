	...
	<div class="login">
		<div class="bg_cont">
			<div class="login_box">
				...
				<div class="get_upd">
					<el-button type="text" @click="getUpd">{{
						$t('common.forgetPassword')
					}}</el-button>
				</div>
				<div class="box_wel">{{ $t('common.welcomeManagement') }}</div>
				<!-- <div class="box_logo">
          <img src="@/assets/img/common/qishu.png" alt="" />
        </div> -->
				<div class="box_logo">
					<img src="@/assets/img/common/jinFuCompany.jpg" alt="" />
				</div>
				<div class="jin_company">{{ $t('common.jinFuCompany') }}</div>
				<div class="box_info">
					<el-form
						class="box_form"
						:model="loginForm"
...
export default defineComponent({
	setup() {
			...
			getLoign() {
				...
				commonApi
					...
					.then(res => {
						...
						if (status === 200) {
							methods
								.checkLoginType(custom_data.user_id, custom_data.token)
								.then(() => {
		...
		const methods = {
			...
			// noLogin() {
			//   const noLogin = getNoLogin() as string
			//   const userInfo = getUserInfo() as string
			//   if (noLogin) {
			//     state.emailCheckboxtype = JSON.parse(noLogin)
			//     if (userInfo) {
			//       let _userInfo = JSON.parse(userInfo)
			//       loginForm.username = _userInfo.username
			//     }
			//   }
			// },
		}
